...
maxLanes = 20; 
expLaneNum = 2;  
Rep_ref   = zeros(expLaneNum, maxLanes); 
count_ref = zeros(1, maxLanes);        
threshold = 75; 
...
while ~isDone(hVideoSrc) 
    ...
    [Rep_ref, count_ref] = videolanematching(Rep_ref, count_ref, ...
                                maxLanes, expLaneNum, Enable, Line, ...
                                threshold, frameFound+frameLost);

    Pts = step(hHoughLines1, Rep_ref(2,:), Rep_ref(1,:), Imlow);

    [TwoValidLanes, NumNormalDriving, TwoLanes, OutMsg] = ...
            videodeparturewarning(Pts, Imlow, maxLanes, count_ref, ...
